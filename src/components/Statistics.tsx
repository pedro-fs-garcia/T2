...
interface EstatisticasState {
  topClientesPorQuantidade: EstatisticasConsumo[];
  itensMaisConsumidos: EstatisticasItem[];
  consumoPorTipoRaca: EstatisticasPet[];
  topClientesPorValor: EstatisticasConsumo[];
  carregando: boolean;
  erro: string | null;
}

class Estatisticas extends React.Component<object, EstatisticasState> {
  private servicoEstatisticas: ServicoEstatisticas;

  constructor(props: object) {
    super(props);
    this.servicoEstatisticas = new ServicoEstatisticas();
    this.state = {
      topClientesPorQuantidade: [],
      itensMaisConsumidos: [],
      consumoPorTipoRaca: [],
      topClientesPorValor: [],
      carregando: true,
  ...
  private async carregarDados() {
    try {
      ...
      const [
        ...
        this.servicoEstatisticas.obterTopClientesPorQuantidade(),
        this.servicoEstatisticas.obterItensMaisConsumidos(),
        this.servicoEstatisticas.obterConsumoPorTipoRaca(),
        this.servicoEstatisticas.obterTopClientesPorValor()
      ]);

      this.setState({
        topClientesPorQuantidade: estatisticasQuantidade,
        itensMaisConsumidos: itensConsumidos,
        consumoPorTipoRaca: estatisticasPet,
        topClientesPorValor: estatisticasValor,
        carregando: false
      });
    } catch (err) {
      this.setState({
        erro: 'Erro ao carregar as estatísticas. Por favor, tente novamente.',
        carregando: false
      });
  ...
  render() {
    const { carregando, erro, topClientesPorQuantidade, itensMaisConsumidos, consumoPorTipoRaca, topClientesPorValor } = this.state;
